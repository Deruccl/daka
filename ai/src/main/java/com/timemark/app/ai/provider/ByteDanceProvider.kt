 ...
 * 字节豆包 Provider
 *
 * 简化实现：采用 OpenAI 兼容格式（火山方舟已提供兼容接口）。
...
class ByteDanceProvider(
    ...
    override val providerType: AIProvider = AIProvider.BYTEDANCE

    private val defaultBaseUrl = "https://ark.cn-beijing.volces.com"
    ...
    override suspend fun chat(request: ChatRequest, config: AIConfig): ChatResponse {
        ...
        val url = "$baseUrl/api/v3/chat/completions"
